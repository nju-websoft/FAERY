...
class HierarchicalTransformer(nn.Module):
    def __init__(
        ...
        self.Prediction1 = nn.Linear(d_model * 2, d_model * 4)
        self.Prediction2 = nn.Linear(d_model * 4, 1)
        self.LeakyReLU = nn.LeakyReLU(0.1)
        self.apply(lambda module: init_params(module, self.L))

    def forward(self, query_features, dataset_features):
        '''linear projection'''
        tensor = self.att_embeddings_nope(query_features)
        # Type Transformer
        type_transformer_output = self.type_transformer(tensor)
        # Hop Transformer with the output of Type Transformer as input
        qh = self.hop_transformer(type_transformer_output)
        qh = qh.mean(dim=1, keepdim=False)
        tensor = self.att_embeddings_nope(dataset_features)
        # Type Transformer
        type_transformer_output = self.type_transformer(tensor)
        # Hop Transformer with the output of Type Transformer as input
        dh = self.hop_transformer(type_transformer_output)
...
class TypeTransformer(nn.Module):
    def __init__(self, type, seq, d_model, ffn_dim, dropout_rate, attention_dropout_rate, num_heads, L_type,
    ...
    def forward(self, x):
        ...
        for k in range(2, self.seq + 1):
            ...
            sub_tensor_trans_flat = sub_tensor_trans.view(sub_tensor_trans.size(0) * sub_tensor_trans.size(1),
                                                          sub_tensor_trans.size(2), sub_tensor_trans.size(3))
        ...
        concatenated_tensor = concatenated_tensor.view(node_self_agg.size(0), node_self_agg.size(1),
                                                       concatenated_tensor.size(-2), concatenated_tensor.size(-1))
        type_output = torch.cat([node_self_agg, concatenated_tensor], dim=2)
        return type_output


class HopTransformer(nn.Module):
    def __init__(self, seq, d_model, ffn_dim, dropout_rate, attention_dropout_rate, num_heads, L_hop,
                 use_gradient_checkpointing):
