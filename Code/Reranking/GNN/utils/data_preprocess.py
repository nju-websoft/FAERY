...
print(device)
model = SentenceTransformer("/home2/jinghe/discovery/datasets/model/stella_en_400M_v5", trust_remote_code=True).cuda()
...
def generate_node_dat(ori_corpus_file, ori_nodes_file, out_node_file):
    with open(ori_corpus_file, 'r') as f:
        nodes = json.load(f)
    ori_nodes = []
    with open(ori_nodes_file, 'r') as f:
        for line in f.readlines():
            ori_nodes.append(line.strip().split('\t'))
    for r in nodes.keys():
        metadata_corpus.append(nodes[r])
    print(len(metadata_corpus))
    dataset_embeds = stella_embedding(metadata_corpus)
    print(dataset_embeds.shape)
    node_id = 0
    all_nodes = []
    assert len(nodes) == len(ori_nodes)
    for r in ori_nodes:
        record = [r[0], r[1], r[2], dataset_embeds[eval(r[0])]]
...
if __name__ == '__main__':
    generate_node_dat('../../data/ntcir_metadata_content_stella/corpus.json',
                      '../../data/ntcir_metadata_content/node.dat',
                      '../../data/ntcir_metadata_content_stella/node_1024.dat')
