...
class data_loader:
    def __init__(self, path, args):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("../../discovery/datasets/model/bge-reranker-v2-m3/", use_fast=True)
        self.reranker = FlagReranker('../../discovery/datasets/model/bge-reranker-v2-m3/', use_fp16=True)
        self.queries = self.load_keywords('queries.json')
        # print(self.queries)
        self.corpus = self.load_corpus('corpus.json')
        # self.pairs = self.load_pairs('pairs.json')
        self.nodes = self.load_nodes()
        self.links = self.load_links()
        self.edge_index, self.edge_type = self.links_to_edge_index_format(self.links)
        if "ntcir" in args.dataset:
            self.fold = "."
        elif "google" in args.dataset:
            self.fold = args.fold
        else:
            self.fold = f"fold{args.fold}"
        self.labels_train = self.load_labels(f'{self.fold}/train.json')
        self.labels_val = self.load_labels(f'{self.fold}/val.json')
        self.labels_test = self.load_labels(f'{self.fold}/{args.mode}.json')
            ...
            with open(os.path.join(self.path, name), 'r') as f:
                for line in f:
                    ...
                    if "Datafinder" in self.path:
                        queries[eval(th[0])] = th[1].strip()
                    else:
                        queries[th[0]] = th[1].strip()
        return queries
    ...
    def load_labels(self, name):
        ...
        """
        labels = {'total': 0, 'data': {}, 'query_info': {}}
        with open(os.path.join(self.path, name), "r") as f:
            json_data = json.load(f)
        bm25_results, bge_input, query_nodes = [], [], {}
        temp_num = 0
        datatype = name.replace('.json', "").replace(f'{self.fold}/', '')
        # if os.path.exists(os.path.join(self.path, f'{self.fold}/query_nodes_{self.bm25_num}_{self.top_k}_{datatype}.json')):
        #     query_nodes = json.load(
        #         open(os.path.join(self.path, f'{self.fold}/query_nodes_{self.bm25_num}_{self.top_k}_{datatype}.json')))
        if os.path.exists(os.path.join(self.path, f'{self.fold}/query_nodes_100_20_{datatype}.json')):
            query_nodes = json.load(
                open(os.path.join(self.path, f'{self.fold}/query_nodes_100_20_{datatype}.json')))
        else:
            with tqdm(total=len(json_data)) as pbar:
                for k, v in json_data.items():
                    ...
                    #     break
                    query_id = k
                    query_keywords = self.queries[query_id]
            ...
            for i, k in enumerate(list(json_data.keys())):
                ...
                # if temp_num > 30:
                #     break
                rerank_result = list(zip([i for i in range(len(bm25_results[i]))],
                ...
                # query_nodes[k] = []
            with open(os.path.join(self.path, f'{self.fold}/query_nodes_{self.bm25_num}_{self.top_k}_{datatype}.json'), "w") as f:
        ...
        with tqdm(total=len(json_data)) as pbar:
            for k, v in json_data.items():
                ...
                #     break
                query_id = k
                query_node_ids = query_nodes[k][:self.top_k]
                labels['query_info'][query_id] = {'query_node_ids': sorted(query_node_ids),}
                # labels['query_info'][query_id] = {'query_node_ids': self.process_text(self.queries[query_id], 16)}
                if 'test' in name:
                    for dataset_node, rel in v.items():
                        if query_id not in labels['data'].keys():
                            labels['data'][query_id] = {}
                        labels['data'][query_id][dataset_node] = rel
                else:
                    for dataset_node, rel in v.items():
                        if query_id not in labels['data'].keys():
                            labels['data'][query_id] = {}
                        if rel > 0:
                            labels['data'][query_id][dataset_node] = 1
                        else:
                            labels['data'][query_id][dataset_node] = 0
    ...
    def links_to_edge_index_format(self, links):
        """
        将links字典转换为edge_index和edge_type格式

        Args:
            links: load_links函数返回的links字典

        Returns:
            edge_index: torch.Tensor, shape [2, num_edges]
            edge_type: torch.Tensor, shape [num_edges]
        """
        all_edges = []
        all_edge_types = []

        # 遍历所有关系类型
        for r_id, adj_matrix in links['data'].items():
            # 将稀疏矩阵转换为COO格式获取边
            coo_matrix = adj_matrix.tocoo()
            edges = np.vstack((coo_matrix.row, coo_matrix.col))  # [2, num_edges_for_this_type]

            # 添加到总边列表
            all_edges.append(edges)

            # 创建对应长度的边类型张量
            edge_types = np.full(coo_matrix.nnz, r_id)  # 用关系ID填充
            all_edge_types.append(edge_types)

        # 合并所有边
        if all_edges:
            edge_index = np.hstack(all_edges)  # [2, total_edges]
            edge_type = np.hstack(all_edge_types)  # [total_edges]
        else:
        ...
        print(f"Generated edge_type shape: {edge_type.shape}")
        print(f"Total edges: {edge_index.shape[1]}")
        print(f"Edge types distribution: {dict(zip(*np.unique(edge_type.numpy(), return_counts=True)))}")
